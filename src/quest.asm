;-----------------------------------------------------------------------------
; MSX2 던전 탐험 - Bard`s Tale / Wizardry 형태 (SCREEN 5 판, **보관용**)
;
; 새 코드는 src/quest3.asm 에 넣는다. 이 파일은 SCREEN 5 판으로 얼려 둔 것이다.
;
; 그래도 계속 빌드되게 두는 이유: verify_quest_sides.ps1 이 던전 화면을 **픽셀
; 하나까지** 파이썬 모델과 맞춰 보는데, 그 모델이 4bpp 기준이라 이 롬에서만
; 돈다. 두 판이 공유하는 하위 파일(questfight/questmap/questtext/...)을 고쳤을
; 때 이 검사를 돌리면 공유 코드의 회귀가 잡힌다.
;
; **놀아 보려고 이 롬을 굽지 말 것.** 게임으로서는 몇 세대 뒤처져 있다.
; 여기 없는 것 (전부 IFDEF SCREEN8 안에 있다):
;
;   대열          무리가 셋이어도 그림 한 장이다. 여러 마리로 안 선다.
;   표적 고르기   칠 상대를 못 고른다. 늘 살아 있는 첫 놈을 친다.
;   HP 게이지     발아래 점이 없다.
;   맞은 표시     섬광도 "-N" 도 없다.
;   섞인 무리     한 무리는 늘 한 종류다.
;   타이틀/음악   --title 자체가 SCREEN 8 빌드에만 있다.
;
; 여기 들어오는 것은 공유 파일에 있으면서 IFDEF 로 안 막힌 것뿐이다 - 글자
; 찍기, 지도 생성, 전투 판정 같은 것. 빌드와 검사의 기준은 quest3 다.
;

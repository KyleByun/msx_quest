...
def _uv_left(x, y):
    ...
    if dx <= 0:
        return None
    z = 36.0 / dx
    return z * TEX_PER_CELL, ((y - G.CY) * z / P + 0.5) * TEX_PER_CELL, z
...
def _uv_floor(x, y):
    ...
    if dy <= 0:
        return None
    z = 36.0 / dy
    return ((x - G.CX) * z / P + 0.5) * TEX_PER_CELL, z * TEX_PER_CELL, z
...
def bake_front(d, x, y):
    ...
    def uv(px, py):
        return (((px - G.CX) * d / P + 0.5) * TEX_PER_CELL,
                ((py - G.CY) * d / P + 0.5) * TEX_PER_CELL, d)
...
#
# 여기까지가 bake_side_face 다. 그런데 이 그림에는 전제가 하나 숨어 있다.
#
#   **대각선 앞칸이 벽이어야 한다.**
#
# 그 칸마저 뚫려 있으면 광선은 z = j+1 을 그냥 지나간다. 그때 밝은 벽면을
# 그려 놓으면 없는 벽이 생긴다. 실제로 광장(벽으로 둘러싸이지 않은 넓은 방)
# 한가운데에 서면 화면 좌우 끝에 있지도 않은 벽이 나타났다. 옆칸 하나만 보고
# 두 칸의 결과를 그린 탓이다 - bake_side_open 이 그 나머지 한 경우다.
#
...
def bake_side_face(j, x, y):
    """옆칸이 뚫렸고 대각선 앞칸은 벽 - 그 칸의 정면이 보인다."""
    d = j + 1
    dy = y - G.CY
...
# **벽이 없다고 치고 바닥과 천장을 그대로 이어 그리면 된다.** 위에서 쓰던
# HALF[j+1] 경계는 z = j+1 에 벽이 있을 때만 의미가 있으므로 여기서는 없다.
#
# 소실점에 가까울수록 z 가 커져 depth_factor 가 0.22 로 깔리므로, 바닥과 천장이
# 옆으로 쓸려 들어가다 어둠으로 사라진다. 뚫린 느낌을 만드는 것은 밝기가 아니라
# 이 "쓸려 들어가는 모양"이다.
#
# 남는 오차 하나는 알고 남긴다. 이것은 두 칸까지만 보는 근사라서, 옆으로 1 앞으로
# j+2 인 칸이 벽이면 원래는 z = j+2 에서 그 정면이 보여야 한다. 어긋나는 자리는
# |dy| < HALF[j+2] 로 좁고 둘 다 같은 회색 벽돌이라 눈에 띄지 않는다. 제대로
# 하려면 실행 중에 광선을 쏘아야 하는데, 그러면 이 렌더러의 전제가 무너진다.
def bake_side_open(j, x, y):
    """옆칸도 그 앞칸도 뚫렸다 - 벽 없이 바닥과 천장만 이어진다."""
    dy = y - G.CY
    if dy == 0:

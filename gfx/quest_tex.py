...
#
#   -> 옆길로 보이는 것은 "대각선 앞칸(오른쪽 1, 앞 j+1)의 정면"이다.
...
#
# 그 바깥 - 옆 통로의 천장과 바닥 - 이 이 그림에서 제일 중요하다.
#
# 한 번은 거기를 검게 두었다. 그랬더니 뚫린 자리 위아래가 커다란 검은 삼각형이
# 되어, 통로가 아니라 벽에 진 그림자로 읽혔다. 뚫린 느낌을 만드는 것은 밝기가
# 아니라 바닥과 천장이 옆으로 쓸려 들어가는 모양이다.
#
# 맞은편 벽도 마찬가지로 어둡게 하면 안 된다. 막다른 곳의 정면 벽과 그 옆의
# 뚫린 자리는 z 가 같은 "한 평면"이다. 한쪽만 어둡게 하면 정면 벽 좌우에 검은
# 띠가 생기는데, 실제로 그 칸까지 걸어가서 보면 밝은 벽이다. 앞뒤가 맞지 않는다.
#
# 그래서 결론은 이렇게 단순해졌다.
#
#   옆길로 보이는 것은 "그 벽이 없었다면 보였을 그림" 그대로다.
#
# 밝기를 손대지 않는다. 그래도 벽과 헷갈리지 않는데, 맞은편 벽은 정면을 향한
# 평면이라 벽돌 크기가 고르고, 옆벽은 비스듬한 평면이라 벽돌이 소실점 쪽으로
# 늘어나기 때문이다. 여기에 바닥/천장이 옆으로 빠지는 모양이 더해진다.
def bake_open(j, x, y):
    d = j + 1
    dy = y - G.CY
